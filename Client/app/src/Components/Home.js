...
function Home() {
    ...
    return (<>
        ...
        <div style={{ display: "flex", alignItems: "center", flexDirection: "column", marginTop: "2.5vh" }}>
            ...
            <h2 style={{ marginBottom: "5vh" }}>Quotes Around the World</h2>

            <div className="btn-group" role="group" aria-label="Basic radio toggle button group">
                <input type="radio" class="btn-check" name="btnradio" id="btnradio1" autocomplete="off" onChange={check} />
                <label class="btn btn-outline-primary" for="btnradio1">All</label>
                <input type="radio" class="btn-check" name="btnradio" id="btnradio2" autocomplete="off" onChange={check}/>
...

            {
                ...
                quotesData.map((data) => {
                    // debugger
                    return <div style={{ width: "75vh", border: "2px solid black", margin: "2.5vh", paddingTop: "1.2vh", paddingLeft: "2vh", paddingRight: "2vh", fontSize: "2.1vh" }}>
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                            {data.text}
                            {
                                data.user_id != sessionStorage.getItem("user_id") &&  !favId.includes(data.quote_id)?
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="flexCheckDefault" onClick={() => {likeQuote(data.quote_id)}} />
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between" }}>
                            {data.text}
                            {
                                // data.user_id != sessionStorage.getItem("user_id") ?
                                    <div class="form-check">
                                        <input class="form-check-input" type="checkbox" id="flexCheckDefault" onClick={() => {unlikeQuote(data.quote_id)}} />
                                        <label class="form-check-label" for="flexCheckDefault">
                                            Unlike
                                        </label>
                                    </div>
